...
uint32_t ThreadPeriod [NUMPERIODIC];
uint32_t Periodic_Thread_Current;
uint32_t Periodic_Counter[NUMPERIODIC];
void(*Periodic_Thread [NUMPERIODIC])(void);
uint32_t Counter;
...
// ****IMPLEMENT THIS****
	if(Periodic_Thread_Current  == NUMPERIODIC) {
		return 0;
	}
	Periodic_Thread[Periodic_Thread_Current] = thread;
	ThreadPeriod[Periodic_Thread_Current] = period;
	Periodic_Thread_Current ++;
  return 1;
}
...
// **RUN PERIODIC THREADS, DECREMENT SLEEP COUNTERS
	LCM = 1;
	for(i=0;i<NUMPERIODIC;i++){
		LCM = LCM*ThreadPeriod[i];
	}
	Counter = (Counter+1)%LCM;
	for(i=0;i<NUMPERIODIC;i++){
		if ((Counter%(ThreadPeriod[i]))== i){
				(*(Periodic_Thread[i]))();
		}
	}
